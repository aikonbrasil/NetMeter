...
def get_iperf_data_single(iperf_out, protocol, streams, repetitions):
    ...
    with open(iperf_out, encoding='utf-8', errors='ignore') as inputfile:
        for line in inputfile:
            ...
            if (len(tmp_lst)>5):
              ...
              print(tmp_lst[-3])
              lat_minimum = tmp_lst[-3]

              print(tmp_lst[-2])
              lat_maximum = tmp_lst[-2]

              print(tmp_lst[-1])
              lat_standard_i = tmp_lst[-1]
              tmp_lst = lat_standard_i.strip().split(' ')
              print(tmp_lst[0])
              lat_std = tmp_lst[0]
            ...
            #  lat_average_i = tmp_lst[-10]
             ...
             # lat_std = tmp_lst[-8]
              ...
              lat_minim = float(lat_minimum)
              lat_maxim = float(lat_maximum)
         ...
         #   for x in tmp_lst:
          #    print(x)
        #    if (len(tmp_lst)>31) and (len(tmp_lst)<33):
              ...
              #iperf_data.append([ lat_average, lat_minim, lat_maxim,  lat_stand])
    ...
    print('Adding info here is OK')
    print(iperf_data.shape)


    ### End connection ammount check
    print('ACHIEVED THIS POINT')
   ...
   # out_arr = np.vstack((iperf_data[:,1].sum(axis=0))).filled(np.nan).T
    print(str(iperf_data[0]))
...
if __name__ == "__main__":
    ...
    tprint('\033  sstarting reading text file  \033')
    init_name = 'UDP_1_st_2020_10_07_06-18-00_one2two_00032B'
    #init_name = 'UDP_1_st_2020_10_07_06-46-28_one2two_00032B'

    protocol='UDP'
    streams=6
    repetitions=6
    print_unit = 'print UNIT'
    cl1_pretty_name = 'name1'
    cl2_pretty_name = 'name2'
    direction= 'one2two'
    test_completed=True
    server_fault = 'too many'
    tcpwin=None
    p=100
    mean_latency = 15

    dir_time='/home/lut/Downloads/git/NetMeter/out/2020_10_07_06-18-00_UDP_1_st/raw-data/'

    image_list='one2two_images'
    ...
    write_gp(init_name + '.plt', basename(init_name + '_iperf_processed.dat'),
            mpstat_single_file, basename(init_name + '.png'),
            mean_latency, protocol, streams, print_unit, cl1_pretty_name,
            cl2_pretty_name, plot_type = 'singlesize', direction = direction,
            finished = test_completed, server_fault = server_fault,
            packet_size = p, tcpwin = tcpwin)
    print('Plotting...')
